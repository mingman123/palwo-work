...
function isM() {
	$(function () {
		...
		var tops = [];
		var stopTime = 400;
		var h2 = $('.wrap1').height() + $('.wrap4').height()/2;
	...
	function changeOpacity() {
		...
		$('.cont22').height(tops[7]-$('.int-box2').height()/2);
		if (top < tops[0]) {
			$(".int-boxs").css({
	...
	function act1() {
		var top = $(window).scrollTop();
		var actTop = $(".cont1").height()-$(window).height()/6+ $(".cont21").height()/2;	// 开始执行动画
		var endTop = window.innerHeight - $(".cont21").height()*8/7 + actTop;	// 动画结束
		//		var actY=window.innerHeight - ((window.innerHeight - $('.bg-pic1').height()) / 2);// 总共要偏移的值
		var actY = window.innerHeight + 2 - $(".cont21").height()*8/7+$(window).height()/5;// 方法二
    ...
    var endTop = tops[7] + stopTime;
		var y = endTop - actY - scrollTop+$(".cont21 .subtitle").height()/3;
	...
	function act3() {
		var top = $(window).scrollTop();
		var actTop = $(".cont1").height() + $(".cont21").height() + $(".cont22").height() + $(".cont3").height() / 2;	// 开始执行动画
		var endTop = $(".cont1").height() + $(".cont21").height() + $(".cont22").height() + $(".cont3").height();	// 动画结束
		var actY = $(".cont3 .subtitle").height() * 2;		// 总共要偏移的值
		var meanY = actY / (endTop - actTop);	// 每滚动一px的时候偏移的值（平均值）
		var num = top - actTop;	// 开始执行动画的滚动值
		var y = num * meanY;	// 实际偏移值    0.6
		if (top < actTop) {
	...
	function act5() {
		...
		var actY = (window.innerHeight - $(".head-title-box").height()) / 2 + $(".head-title-box").height();
		var meanY = actY / (endTop - actTop);
		var num = top - actTop;
		var y = num * meanY;
	...
	function act7() {
		...
		var endTop = window.innerHeight- $(".cont21").height()+actTop;
		// var endTop2 = window.innerHeight - $(".cont21").height() + $(".cont22").height();//方法二
		//		var actY =window.innerHeight;	
