function indexjs() {
  ...
  if (!lolCommon.isPC()) {
    need("daoju.mo", function(mo) {
      var _share = {
        ...
        "icon": "https://game.gtimg.cn/images/lolriotmall/act/a20200818godjax/share.jpg",
  ...
  // 预加载图片
  var imgs = [
    '//game.gtimg.cn/images/lolriotmall/act/a20200818godjax/header-btn1-on.png',
    '//game.gtimg.cn/images/lolriotmall/act/a20200818godjax/header-btn2-on.png',
    '//game.gtimg.cn/images/lolriotmall/act/a20200818godjax/header-btn3-on.png',
  ]
  imgs.forEach((img) => {
    var el = new Image()
    el.src = img
    ...
    if (lolCommon.isPC()) {
      ...
      if (isOfficial) {
        ...
        if (this.dataset.linkId) {
          if (isMP) {
            // 小程序
            wx.miniProgram.redirectTo({url: '/pages/detail/detail?id=' + this.dataset.linkId})
          } else {
            if ($(this).hasClass('pintuan')) {
          ...
          if (isMP){
            wx.miniProgram.redirectTo({url: '/pages/receive/receive'})
            // alert("暂未开放领取");
          }else{
